...
pub async fn run(
    ...
    let result: Result<()> = loop {
        ...
        if let Err(e) = terminal.draw(|frame| render::draw(frame, &app, &snapshot)) {
            break Err(e.into());
        }
        // ratatui has no concept of a clickable link, so the real OSC 8 hyperlink for the
        // footer is written directly to the terminal after each frame, on top of the plain
        // text ratatui just drew — self-healing every frame regardless of ratatui's internal
        // diffing/redraw behavior. Skipped while the QR popup may be covering that row.
        if !app.show_qr {
